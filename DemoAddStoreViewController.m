...
@interface DemoAddStoreViewController ()
...
@implementation DemoAddStoreViewController
...
- (void)viewDidLoad
{
    ...
    if (IS_IPAD) {
        ...
        [titleDirection setFrame:CGRectMake(200, 64, 380, 70)];
        titleDirection.text = @"Enter your items for your store";
        ...
        [titleDirection setFrame:CGRectMake(headerImageView.frame.origin.x+70, headerImageView.frame.origin.y+30, headerImageView.frame.size.width-70, 70)];
...
-(void)addToParseAction{
    ...
    NSSortDescriptor *objectDescriptor = [[NSSortDescriptor alloc] initWithKey:@"objectId" ascending:YES];
    NSArray *sortDescriptors = @[objectDescriptor];
    request.sortDescriptors = sortDescriptors;
    NSArray *storeObjArray = [_demoManagedObjectContext executeFetchRequest:request error:&error];
    NSMutableArray *storeItems = [[NSMutableArray alloc]initWithArray:storeObjArray];
    DemoStoreObject *lastObject = [storeItems lastObject];
    self.objectStoreCount = [lastObject.objectId intValue];
    ...
    for (int j = 0; j<_i; j++) {
        ...
        if ( ![sItem.text isEqualToString:@""] && ![sCost.text isEqualToString:@""] ) {
           ...
           DemoStoreObject *storeItem = [DemoStoreObject createStoreObjectInCoreWithDictionary:@{@"objectId":[NSString stringWithFormat:@"%d",self.objectStoreCount],@"item":storeItemName,@"cost":costNum,@"userType":@"Teacher"} inManagedObjectContext:_demoManagedObjectContext];
            [totalItemsValue addObject:storeItem];
    ...
    if ([totalItemsValue count]!=0) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Oops, did you forget add an item?" message:@"Make sure you type something in the lined paper." delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alert show];
    }
