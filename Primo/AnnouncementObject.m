...
-(void)postAnnouncementTo:(NSString*)recipient announcementType:(NSString*)aType personType:(NSString*)typeOfPerson announcementBody:(NSString*)aBody{
    ...
    if (_teacherId !=nil) {
        ...
        [insert saveIntoDatabaseInBackgroundWithBlock:^(NSError *error) {
            if (error) {
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Post failed." message:@"You must have a network connection to post a broadcast" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil];
                [alert show];
            }
        }];
