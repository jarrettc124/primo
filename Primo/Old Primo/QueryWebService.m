...
-(void)setLimit:(NSInteger)limit whereOffset:(NSInteger)offSet{
    ...
    self.endSql = [self.endSql stringByAppendingFormat:@" LIMIT %ld OFFSET %d",(long)limit,offSet];
...
-(void)findAllObjectsInRowWithCompletion:(void(^)(NSError* error,NSArray* rows))block{
    ...
    if(self.endSql !=nil){
        ...
        NSMutableURLRequest *request = [[NSMutableURLRequest alloc] init];
        [request setURL:[NSURL URLWithString:@"http://www.pixelandprocessor.com/stanton/query.php"]];
        [request setHTTPMethod:@"POST"];
        [request setValue:postLength forHTTPHeaderField:@"Content-Length"];
        [request setValue:@"application/x-www-form-urlencoded" forHTTPHeaderField:@"Content-Type"];
        [request setHTTPBody:postData];
