...
@property(nonatomic) UITextField *editCoinTextField;
...
-(void)showCoinView{
    ...
    if (![self.coinView isDescendantOfView:self.view]) {
        ...
        if (IS_IPHONE){
            ...
            [self.view addConstraint:[NSLayoutConstraint constraintWithItem:self.coinView attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:self.view attribute:NSLayoutAttributeCenterY multiplier:1 constant:-120]];
        ...
        [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
            ...
            //        [self.coinView addSubview:coinsLabel];
...
-(void)enterIntoDatabase{
    ...
    NSMutableArray *totalStudentsValue = [[NSMutableArray alloc]initWithCapacity:0];
    NSString *objId = [[NSUserDefaults standardUserDefaults] objectForKey:@"ObjectId"];
    ...
    for (UITextField *texting in _studentNames){
        if( ![texting.text isEqualToString:@""]){
            ...
            NSArray *studentValue = @[studentName,objId,[[NSUserDefaults standardUserDefaults] objectForKey:@"Email"],[NSString stringWithFormat:@"%d",texting.tag],[self.className lowercaseString],[NSString stringWithFormat:@"%@",@([self.editCoinTextField.text intValue])]];
            [totalStudentsValue addObject:studentValue];
            
        }
    }
    
    if ([totalStudentsValue count]!=0) {
        
        //check to see if class is made if not, make it

        
        InsertWebService *insertData = [[InsertWebService alloc]initWithTable:@"StudentObject"];
        [insertData insertMultipleObjectsInColumnsWhere:@[@"studentName",@"teacher",@"teacherEmail",@"studentNumber",@"className",@"coins"] setObjectsValues:totalStudentsValue];
        [insertData saveIntoDatabaseInBackgroundWithBlock:^(NSError *error) {
            if(!error){
                
                NSLog(@"delegate here");
                if(self.isNewClass){
                    [self.delegate addNewClassToDatabaseClassName:self.className];
                }else{
                    [self.delegate refreshAfterDoneAdding:self];
                }
